...
local function main()
	...
	if type(file) ~= "userdata" then
		print("Unexpected error running sandboxed code: file error")
		return
	end

	local script,err = file:read("*all")
	if type(err) ~= "nil" then
		print("Unexpected error running sandbox code: read error")
		return
	end
	...
	local file = false--open("/tmp/webluasession-"..session, "r")
	if file then
	...
	if results[1] then
		...
		local file = false--open("/tmp/webluasession-"..session, "w+")
		if file then
